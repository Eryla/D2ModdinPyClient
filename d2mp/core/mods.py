...
class ModManager(object):
	...
	def get_active_mod(self):
		...
		debug_fancy_message("Current active mod: %s" %name)
		return name
...
#         pass
	...
	def delete_mod(self, mod_name, version = None):
		...
		if not exists(mod_path): 
			...
			return
		debug_fancy_message("deleting mod %s" %(mod_name))
		rmtree(mod_path)
	...
	def delete_mods(self):
		...
		debug_fancy_message("deleted all present mods")
		self._cache["mods"] = []
	...
	def set_mod(self, mod_name):
		...
		debug_fancy_message("Setting active mod to %s." %mod_name);
	...
	def _extract_mod_version(self, addon_dir):
		...
		debug_fancy_message("Found mod %s, detecting version..." %(addon_dir))
		file_path = join(self._d2mp_path(), join(addon_dir, "addoninfo.txt"))
		if not isfile(file_path): 
			debug_fancy_message("no addoninfo file found: %s" %(file_path))
			return "0.0.1"
	...
	def mod_names_as_string(self):
		...
		if mod_names:
			msg = "Mods installed: %s" %(", ".join(mod_names))
		else:
